...
for (event_stream_per_student,c_name,c_time) in zip(event_stream_per_students,course_name,course_time):
    ...
    passing_list=process_utils.whether_passing(event_stream_per_student,c_name+'-'+c_time)
    
    train_data= defaultdict(list)
    pass_label= defaultdict(list)
    ...
    for u_name, actions in event_stream_per_student.items():
        ...
        if u_name in passing_list:
            pass_label[u_name]=[1]*(len(train_data[u_name]))
        else:
            pass_label[u_name]=[0]*(len(train_data[u_name]))
     
    print('finishing one.....' )           
    train_sets.append(train_data)
    pass_sets.append(pass_label)
...
for i in range(0,len(train_sets)):
    ...
    pass_df = pd.DataFrame({'username': list(pass_sets[i].keys()), 'seq': list(pass_sets[i].values())})
    pass_list = pass_df['seq'].values
    
    # make the directory for passing file
    course_dir=keras_file+'/'+course_name[i]+'-'+course_time[i]
    course_train_dir=course_dir+'/train'
    pass_dir=course_train_dir+'/pass'
    if not path.exists(course_dir):
        os.mkdir(course_dir)
    if not path.exists(course_train_dir):
        os.mkdir(course_dir)
    if not path.exists(pass_dir):
        os.mkdir(pass_dir)
    ...
    for k in range(0,count+1):
        ...
        if k==count:
            ...
            t_pass=pass_list[10000*k:10000*(k+1)]
        
        #padding
        t_x_train = sequence.pad_sequences(t_event, maxlen=max_seq_len, dtype='bool',padding='post', truncating='post')
        t_y_pass=sequence.pad_sequences(t_pass,maxlen=max_seq_len, dtype='bool',padding='post', truncating='post')

        np.save(pass_dir+'/x'+str(k)+'_train',t_x_train)
        np.save(pass_dir+'/ori_length'+str(k),t_ori_length)
        np.save(pass_dir+'/week_label'+str(k),t_week_label)
        np.save(pass_dir+'/y'+str(k)+'_pass',t_y_pass)
